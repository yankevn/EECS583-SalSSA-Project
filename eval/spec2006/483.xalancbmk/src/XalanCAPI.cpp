...
#include <string>
#include <sstream>
#define istrstream stringstream
			...
			XalanHandle		theXalanHandle)
{
	...
	if (status == 0)
	{
		...
		//*theOutput = theOutputStream.str().c_str();
		strcpy(*theOutput,theOutputStream.str().c_str());
	}
			...
			XalanCSSHandle*		theCSSHandle)
{
	...
	istrstream	theInputStream( std::string(theXSLStream,theXSLStreamLength) );
			...
			XalanPSHandle*	thePSHandle)
{
	...
	istrstream	theInputStream( std::string(theXMLStream, theXMLStreamLength) );
